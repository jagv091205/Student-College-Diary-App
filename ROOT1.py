...
# Define new_func1 to create and return a new frame
def new_func1(parent_frame):
    return tk.Frame(parent_frame)

# Assuming button_frame is defined somewhere in your code
button_frame = tk.Frame()  # You need to define button_frame before using it

# Define create_button1 function
def create_button1(image_path, row, column, command_func=None):
    ...
    # Create a separate frame for each button
    button_frame_inner = new_func1(button_frame)
    button_frame_inner.pack(side=tk.LEFT, padx=5, pady=5)
...
def open_attendance_window():
    ...
    back_arrow_image = Image.open("images/back_arrow.jpg")
...
# image = Image.open(image_path)
# image = image.resize((400, 533))  # Resize image
# photo = ImageTk.PhotoImage(image)
